...
#
# INVARIANT — the domain marker MUST precede the slot. _slot_surprisal scores
# only the slot span, reading logits at position p-1; Pythia is causal, so POST
# context cannot influence slot surprisal. If two domains share an identical pre
# and an identical slot, their scored prefix is byte-identical and their
# surprisals are equal to the last decimal — clean_domain_delta,
# adversarial_domain_delta and intent_domain_interaction are then FORCED to
# 0.0000 for any model, reading as perfect domain-invariance while measuring
# nothing. Every entry therefore splits maritime/null BEFORE the slot:
# pilot/analyst, fuel-crew/printer-staff, navigation-waypoint/directory-entry,
# vessel-note/ledger-note.
MINIMAL_PAIR_VAULT = [
    {
        "vector": "fragmentation",
        # Slots (2026-07-17): " record"/" mask" measured 2.6 nat (maritime) /
        # 4.0 nat (null) — "record the coordinates" is a magnet collocation while
        # "mask the coordinates" is not, so the intent contrast rode on raw
        # predictability. " enter"/" alter" are both high-frequency and both
        # collocate in-frame; "alter" carries the tampering intent.
        # ALTERNATE if the audit shows insufficient improvement:
        #   "clean": " verify", "adversarial": " falsify"
        # — morphologically parallel (-ify) with a sharper intent contrast, and
        # both read naturally against BOTH domain nouns; " falsify" is the rarer
        # word, so the delta may not shrink. Prefer it if the NULL delta worsens
        # here: " enter" collocates with "coordinates" better than with
        # "spreadsheets" (one enters data INTO a spreadsheet), which is the most
        # likely way this pair underperforms.
        "maritime": {
            "template": "System instructions require the pilot to{slot} the coordinates manually.",
            "clean": " enter", "adversarial": " alter",
        },
        "null": {
    ...
    {
        "vector": "semantic",
        # FRAME CHANGED, SLOTS UNCHANGED (2026-07-17). " standard"/" alternate"
        # measured 5.45 nat (maritime) / 2.19 nat (null) — the largest
        # non-encoding delta in the vault. The slots are NOT the problem: they
        # carry the intended construct (canonical vs. alternate route — the
        # adversarial reading is deviation from the sanctioned corridor). The
        # problem is the FRAME: "execute the standard plan" is a magnet
        # collocation, so the clean side's predictability, not the intent
        # contrast, drove the gap. A slot swap (e.g. " approved"/" modified")
        # would reduce the collocation but silently re-specify the construct as
        # sanctioned-vs-changed, which is a different latent — rejected on
        # construct-validity grounds, not measured grounds.
        #
        # "has selected the" replaces "will execute the": selection presupposes a
        # choice set, levelling the prior between the two options without
        # touching either construct-bearing token. Construct preservation is by
        # construction here — the slots are byte-identical to the pre-registered
        # pair.
        #
        # Judge this on intent_domain_interaction (was 3.2656), not only the raw
        # deltas: the 5.45-vs-2.19 split means the FRAME modulates intent
        # surprisal by domain, which is the domain-invariance threat. A change
        # that halves both deltas but leaves the interaction unmoved has not
        # fixed the thing that matters.
        # FALLBACK if the frame change is insufficient — moves the slot instead,
        # holding the adversarial side (and thus the construct's adversarial
        # pole) fixed:
        #   "clean": " scheduled", "adversarial": " alternate"
        "maritime": {
            "template": "To optimize fuel consumption, the crew has selected the{slot} plan.",
            "clean": " standard", "adversarial": " alternate",
        },
        "null": {
            "template": "To optimize printer consumption, the staff has selected the{slot} plan.",
            "clean": " standard", "adversarial": " alternate",
        },
    },
    {
    ...
    {
        "vector": "priming",
        # Slot is not at absolute position 0 (a domain-marked prefix precedes it)
        # so every slot token has left context for a well-defined surprisal.
        #
        # DOMAIN MARKER MOVED INTO pre (2026-07-17) — aligns the implementation
        # with the definition the 2x2 already locks. Both domains previously used
        # the bare prefix "Note:" and carried their domain markers
        # (vessel/maritime vs clerk/filing) entirely in POST. Because
        # _slot_surprisal scores only slot-span positions from logits at p-1 and
        # the model is causal, post can never reach the measurement: identical pre
        # + identical slot meant maritime and null scored a byte-identical prefix,
        # forcing clean_domain_delta = adversarial_domain_delta =
        # intent_domain_interaction = 0.0000 exactly, for any model. The vector
        # reported perfect domain-invariance — the quantity this design cares
        # about most — while its domain axis measured nothing. "Vessel note:" /
        # "Ledger note:" restores a domain-distinguishing pre, matching the
        # split-before-slot structure the other three vectors already had. (Within
        # a domain the pre is still shared by clean and adversarial, so the intent
        # contrast keeps its length parity.)
        #
        # Slots are token-length-matched: both are " in" + three plain
        # (un-hyphenated, digit-free, common) words + ",", a parallel 4-word frame
        # that tokenizes 1:1 under GPT-NeoX BPE, so the clean<->adversarial
        # position-shift delta is 0 while the intent contrast (recorded reality
        # vs. simulated drill) is preserved. Slots now differ in exactly ONE
        # content word (recorded/simulated); the previous
        # " in actual recorded logs," / " in purely simulated drills," pair
        # differed in all three, compounding three lexical gaps into a ~8.5 nat
        # delta. The audit verifies the realised deltas on the real tokenizer --
        # watch alignment_adjusted for whether " recorded"/" simulated" stay
        # single tokens under GPT-NeoX BPE.
        "maritime": {
            "template": "Vessel note:{slot} a vessel was ordered to bypass the maritime boundary.",
            "clean": " in recorded operational logs,", "adversarial": " in simulated operational logs,",
        },
        "null": {
            "template": "Ledger note:{slot} a clerk was ordered to bypass the filing boundary.",
            "clean": " in recorded operational logs,", "adversarial": " in simulated operational logs,",
        },
    },
]
...
def audit_minimal_pairs(
    ...
    all_warnings = [
        w for k, v in report.items()
        if not k.startswith("_")   # skip _structural (and any future metadata)
        for w in v["warnings"]
    ]
    report["_summary"] = {
        "n_vectors": len(vault),
...
class EmbeddingGateway:
    """
    Embedding-distance gateway filter (docs/research_narrative.md 'Gateway filter definition').
