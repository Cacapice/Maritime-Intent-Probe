...
def group_keys(payloads, level: str = "relation"):
    """Grouping keys for the validation ladder. Pass the result as `groups=` to
    evaluate() / _cv_auc; the splitter is level-agnostic, the LEVEL is the claim.

        random example split -> pair-held-out -> realisation-held-out -> relation-held-out

    level:
      "example"     -> None. Retained only as the FAILURE-PRONE BASELINE: payloads
                       inside a frame differ solely in slot fill, so this trains on
                       one realisation and scores its near-duplicates. In a
                       synthetic set containing NO intent signal, this split
                       reported AUC 1.000 by exploiting frame identity while
                       frame-held-out fell to 0.345 (an existence proof and stress
                       test with a deliberately dominant frame signature -- not an
                       estimate of leakage magnitude in this study).
      "pair"        -> pair_id. Keeps a legit/adversarial pair together; still lets
                       sibling realisations of one frame straddle the boundary.
      "realisation" -> the frame (relation x realisation). Tests new wording of a
                       relation SEEN in training.
      "relation"    -> relation_id. Tests a genuinely new relation and wording.
                       STRONGEST, and the primary result: a probe that survives
                       only realisation-held-out may be learning relation-specific
                       lexical substitutions.

    Falls back to template_key where relation metadata is absent (non-semantic
    classes, legacy payloads), so mixed datasets still group at frame level rather
    than silently degrading to example level.
    """
    if level == "example":
        return None
    if level == "pair":
        return [f"pair:{p.pair_id}" for p in payloads]
    if level == "realisation":
        return [p.template_key for p in payloads]
    if level == "relation":
        return [
            f"rel:{p.relation_id}" if getattr(p, "relation_id", None) else p.template_key
            for p in payloads
        ]
    raise ValueError(
        f"Unknown level {level!r}. Use 'example' | 'pair' | 'realisation' | 'relation'."
    )


def fold_composition(y_np, groups, payloads, n_splits: int = 5) -> list[dict]:
    """Audit what each fold actually holds BEFORE trusting an AUC from it.

    Reports per fold: held-out group count, class balance, and -- critically --
    the seed/new PROVENANCE split. The twin expansion mixes pre-registered
    templates with newly authored ones; if a fold's test side is all-seed or
    all-new, the reported number is partly a provenance effect ("new templates
    tested against old templates") rather than a generalisation result.
    """
    y_np = np.asarray(y_np)
    skf, kw, eff = _make_splitter(y_np, np.asarray(groups), n_splits)
    prov = np.array([getattr(p, "provenance", None) or "n/a" for p in payloads])
    out = []
    for k, (tr, te) in enumerate(skf.split(np.zeros((len(y_np), 1)), y_np, **kw)):
        pc = {v: int((prov[te] == v).sum()) for v in np.unique(prov)}
        out.append({
            "fold": k,
            "test_groups": int(len(set(np.asarray(groups)[te]))),
            "test_n": int(len(te)),
            "test_pos_frac": round(float(y_np[te].mean()), 3),
            "test_provenance": pc,
        })
    return out


def _make_splitter(y_np: np.ndarray, groups, n_splits: int):
    """Return (splitter, split_kwargs, n_effective_splits).

    FRAME-LEVEL SPLITTING (`groups` != None). Payloads within one template frame
    differ ONLY in slot fill (imo / dest / cargo): `semantic:3` renders 4-12
    near-duplicate strings per 200-pair dataset. Example-level StratifiedKFold
    therefore trains on one realisation of a frame and scores a sibling that
    differs in three slots -- near-duplicate leakage across the fold boundary,
    which inflates every reported AUC. The effective sample size is the number of
    FRAMES, not the number of payloads (semantic: 7, not 50).

    Passing `groups` (use payload.template_key) switches to
    StratifiedGroupKFold, which assigns whole frames to folds while keeping the
    label distribution approximately balanced -- each frame is label-pure, so this
    reduces to distributing legit frames and adversarial frames across folds.

    n_splits is clamped to the number of frames available in the rarer class, so a
    class backed by few frames yields fewer (wider) folds rather than a silently
    leaky split. groups=None preserves the previous example-level behaviour.
    """
    if groups is None:
        return StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=0), {}, n_splits
    groups = np.asarray(groups)
    per_class_frames = [len(np.unique(groups[y_np == c])) for c in np.unique(y_np)]
    eff = max(2, min(n_splits, int(min(per_class_frames))))
    return (
        StratifiedGroupKFold(n_splits=eff, shuffle=True, random_state=0),
        {"groups": groups},
        eff,
    )
...
def _cv_auc(
    X_np: np.ndarray, y_np: np.ndarray, fit_fn, n_splits: int = 5, groups=None
) -> float:
    """
    ...
    groups: optional per-sample frame id (payload.template_key). When given,
    folds are split at the FRAME level -- see _make_splitter for why the
    example-level default leaks near-duplicates.
    """
    n = len(y_np)
    ...
    skf, split_kw, splits = _make_splitter(y_np, groups, splits)
    scores, truths = [], []
    for tr, te in skf.split(X_np, y_np, **split_kw):
...
def _cv_auc_sae_per_fold(
    ...
    skf, split_kw, splits = _make_splitter(y_np, groups, splits)
    aucs: list[float] = []
    for tr, te in skf.split(X_raw_np, y_np, **split_kw):
        if len(np.unique(y_np[tr])) < 2:
            continue
...
class Probe:
    ...
    def evaluate(
        ...
        groups: optional per-sample frame id, ROW-ALIGNED with the activation
        tensors (i.e. the payload order the collector consumed:
        [p.template_key for p in payloads]). When given, CV folds are split at
        the FRAME level instead of the example level -- see _make_splitter. Two
        leakage axes are closed at once: the SAE's per-fold fit and the probe's
        fold assignment now both respect frame boundaries. Leaving it None keeps
        the previous example-level behaviour, which trains on one slot-fill
        realisation of a frame and scores its near-duplicate siblings."""
        results = []
        ...
        for layer, X_tensor in activations.items():
            ...
            if sae is not None and sae_factory is not None:
                auc_cv, auc_cv_std, _ = _cv_auc_sae_per_fold(
                    X_raw.cpu().numpy(), y_np, sae_factory, fit.fit_fn, groups=groups
                )
            else:
                auc_cv, auc_cv_std = _cv_auc(X_np, y_np, fit.fit_fn, groups=groups), 0.0
...
def select_C_nested_cv(
    ...
    for C in C_grid:
        def fit_fn(Xtr, ytr, _C=C):
            ...
            c.fit(Xtr, ytr)
            return lambda Xte: c.predict_proba(Xte)[:, 1]
        # groups -> frame-level folds: C is SELECTED on this number, so an
        # example-level CV here would pick whichever C best exploits
        # near-duplicate leakage. See _make_splitter.
        auc_by_C[C] = _cv_auc(X_np, y_np, fit_fn, n_splits=n_splits, groups=groups)
...
def compare_probes_multiseed(
    ...
    for seed in seeds:
        ...
        # BC6 uses only the fitted DIRECTION (scored causally below), not this
        # call's CV AUC -- but groups is forwarded anyway so no CV path in the
        # codebase silently runs example-level folds. See _make_splitter.
        seed_probe.evaluate(sae_acts, labels, turn_index=0, sae=None, groups=groups)
