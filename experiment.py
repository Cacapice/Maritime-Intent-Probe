...
import probe
from probe import MassMeanProbe, LogisticRegressionProbe, ProbeResult, compare_probes
import sae
...
from stats import wilson_ci as _wilson_ci, normal_ci as _normal_ci
...
class ProbeExperiment:
    ...
    def run(self, n_pairs: Optional[int] = None) -> dict:
        ...
        for turn_idx in range(self.max_turns):
            ...
            for layer, X in mean_acts.items():
                ...
                mm_results += self.probe.evaluate(
                    {layer: X}, labels, turn_index=turn_idx, sae=sae, sae_factory=sae_factory
                )
            ...
            for layer, X in mean_acts.items():
                raw_mm_results += self.raw_probe.evaluate(
                    {layer: X}, labels, turn_index=turn_idx
                )
        ...
        self.probe.evaluate(
            ...
            turn_index=best_turn, sae=sae_best,
        )
    ...
    def _raw_space_comparison(
        ...
        if raw_result_mm is None:
            ...
            raw_result_mm = self.raw_probe.evaluate(
                {best_layer: X_best}, labels, turn_index=best_turn
            )[0]
    ...
    def _pooling_band(self, best_layer, best_turn, mean_acts, final_acts, labels) -> dict:
    ...
    def refine_layers(self, report: dict, span: int = 2, n_pairs: Optional[int] = None) -> dict:
        ...
        for layer, X in mean_acts.items():
            ...
            res = self.probe.evaluate({layer: X}, labels, turn_index=-1, sae=sae)[0]
            self.lr_probe.evaluate({layer: X}, labels, turn_index=-1, sae=sae)
