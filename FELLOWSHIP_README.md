...
explanations remain consistent with them?* Core analyses (completion
criteria) are E1 and E3–E6; E2 is conditional on compute.

- **E1 — LayerNorm mechanistic test.** Apply the model's learned LayerNorm
  (gain and bias) to residual activations; measure class separation before
  and after, held-out probe AUC, and raw and normalized centroid distance.
  Directly evaluates the LayerNorm-attenuation working interpretation.
- **E2 — Ambient residual-geometry baseline** *(conditional)*. Activation
  norm, covariance anisotropy, eigenspectrum, centroid growth, and
  random-direction separability on unrelated text, as an external reference
  for the depth profile. Deferred to Phase II if compute does not permit,
  without affecting the interpretation of the other Phase I analyses.
- **E3 — Scale-controlled geometry.** Norm-normalized centroid distance,
  Fisher ratio, and Mahalanobis distance, separating activation-scale growth
  from scale-invariant separation.
- **E4 — Formal statistical modeling.** Mixed-effects models wherever
  ...
  with regression estimates and confidence intervals for probe AUC, centroid
  separation, normalized separation, and random-direction separability;
  generalized or non-linear models substituted, and documented, where
  linearity fails.
- **E5 — Continuous geometry analyses.** Individual-observation
  ...
  SAE feature space.
- **E6 — Null-leakage modeling.** Whether representational displacement
  predicts null leakage across perturbation families — testing whether the
  abbreviation family occupies the endpoint of a continuous relationship
  rather than standing as an isolated exception.
...
basis, not independent validation. No Phase I result will be interpreted as
evidence regarding adversarial intent; positive findings characterize the
geometry induced by the present stimuli, and all semantic or causal claims
are reserved for identifiable designs. Where multiple mechanisms remain
compatible with the observations, the least committal interpretation
consistent with the evidence is favored. Failure to discriminate among
mechanisms is itself informative, motivating causal experimentation.
...
- **V3 — Cross-architecture replication.** Principal analyses on at least
  one transformer family beyond Pythia.
