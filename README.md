...
## Relevance to the 2026 agentic-security landscape

The security conversation around language models has shifted from harmful text to harmful *actions* — jailbreaks and indirect prompt injection against agents that execute code and route real-world operations (OWASP Top 10 for Agentic Applications 2026, released Dec 2025; *Careful Adoption of Agentic AI Services*, joint NSA/CISA/ACSC/NCSC international guidance, Apr 2026) — with activation probes for deception and harmful intent proposed as runtime safeguards on the strength of held-out performance (e.g., MacDiarmid et al., 2024; Goldowsky-Dill et al., 2025). This study documents a failure mode directly relevant to that proposal: a probing pipeline can produce robust, depth-stable, held-out geometric separation — passing permutation nulls, random-direction baselines, and cross-basis checks — under a stimulus design in which the intended construct is unidentifiable by any estimator (**BC1**), so evidence of this kind cannot by itself establish that an internals-based monitor measures intent rather than surface form.

The same identifiability requirement transfers structurally to probe-based deception detection — whether a model's output contradicts what it internally represents to be true, or to be its intended action — because instructed-honest versus instructed-deceptive training contrasts share the BC1 confound (the instruction co-varies exactly with the label while the internal-state construct is assumed rather than measured), a training-contrast question distinct from the off-distribution evaluations existing work already employs. More broadly, the study sits within the science of evaluations: construct validity as the precondition for decision-relevant probe metrics, for predictive-validity claims (which this study does not make; the identifiable designs **V1–V3** would make them testable), and for verifying interventions that change what a system learns rather than what it says — with the locked hypotheses, amendment trail, and outcome-independent reporting operationalizing an evidence standard for when results do, and do not, justify semantic interpretation.
...
- A bounded exploratory analysis program (**E1–E7**) discriminating among competing geometric and mechanistic explanations (E2 and E7 conditional on compute)
...
**Exploratory program (OSF Amendment 1).** Bounded and non-gating: existing dataset only, no stimulus modification, no analyses added without a further amendment, and no result interpreted as evidence regarding intent. Key observations: held-out probe AUC is ≈0.76 vs a permutation null (≈0.50 ± 0.06), roughly flat across depth; raw centroid separation grows ~2.3× alongside representational norm (native geometry, not normalized separation — scale-controlled measures are prespecified as **E3**); random directions recover an increasing fraction of the contrast with depth, consistently across raw residual and SAE space. These diagnostics are not statistically independent — they interrogate the same representations — but they progressively rule out probe-specific artifacts (dependence on a single learned readout, instability across random baselines, dependence on one representation basis) and collectively shift the burden of explanation toward properties of the representation itself. An NFKC rescue collapses the four orthographic families (homoglyph, fullwidth, zero-width, delimiter) to chance and is silent on the rest, which stay confounded per BC1; abbreviation is the boundary case, with **E6** testing directly whether its null leakage falls within the range predicted by the displacement–leakage relationship estimated under **E5**, or deviates as an outlier. These observations characterize the geometry induced by the present stimuli only; they do not identify the semantic variable responsible for it.

## Current mechanistic interpretation

Several explanations remain compatible with the observed geometry: activation-scale growth, LayerNorm-mediated attenuation, ambient residual expansion, covariance anisotropy, distributed versus axis-specific representation, and — conditional on **E7** — whether the surface-associated geometry is causally load-bearing for downstream computation or representationally present but behaviorally inert. The exploratory program discriminates among them where possible — the LayerNorm reading directly via **E1**, scale versus geometry via **E3**, formal depth modeling via **E4**, ambient baselines via **E2** (conditional on compute), observation-level relationships via **E5**, the abbreviation outlier check via **E6**, causal ablation via **E7** (conditional on compute) — adopting the least committal interpretation consistent with the evidence. Reporting: effect sizes, 99% confidence intervals, and all prespecified analyses regardless of outcome. E7 is an analysis of the existing maritime stimuli only — activation patching between paired conditions already characterized under BC1, not new data collection — and, like every Phase I analysis, bears on geometry, not on adversarial intent, regardless of outcome. Where compute permits, E7 also compares the 1.4B and 6.9B models within the Pythia family; replication in a transformer family beyond Pythia remains Phase II (**V3**). All semantic claims about adversarial intent remain reserved for the identifiable designs of the validation program below, per Amendment 1.
...
- **V2** — Orthogonal Intent / Action-Space dataset separating surface manipulation from downstream policy, the variable targeted by goal-hijacking and indirect-injection attacks.
- **V3** — Cross-architecture replication beyond the Pythia family. (Within-Pythia scale comparison, 1.4B vs. 6.9B, is addressed conditionally in Phase I by **E7**, not here.)
...
- **Landscape positioning & relation to adjacent work (dated, non-scientific)** — [`POSITIONING.md`](POSITIONING.md)
...
Stimuli: [`environment.py`](environment.py) · CVG suite: [`bias_controls.py`](bias_controls.py), [`null_validator.py`](null_validator.py) · geometry: [`surface_geometry.py`](surface_geometry.py) · causal ablation (Phase I, conditional on compute — **E7**): [`patching.py`](patching.py). Synthetic payloads test detectors offline against open-weight models; no real systems are targeted.
